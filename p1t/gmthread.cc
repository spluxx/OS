...
////////////////////////// Shared States //////////////////////////
static int numtinit = 0; // counts the number of thread initialization
...
// ucontext storages
static ucontext_t *running, *tmp, *to_delete, *to_switch, *to_init; 
...
static map<lock_t, bool> waiting_status;
static map<lock_t, bool> broadcasting_status;
...
///////////////////////////////////////@@@@@@@@@@@@@@@@@@@@2
int switch_context(){                 /// Idea: 1. check readQ count; if it isn't empty,
	interrupt_disable();
	return 0;
}
...
int thread_libinit(thread_startfunc_t func, void *arg) { 
	interrupt_disable();                             //////@@@@@@@@@@
	numtinit += 1; 								 	 //////@@@@@@@@@@
	if(numtinit>=2){interrupt_disable(-1)}   	     //////@@@@@@@@@@
	
	// Initiale thread
    if(tmp != NULL){interrupt_enable(-1)};
    running = tmp = NULL;
    complete_threads = 0;
...
int thread_signal(lock_t lock, lock_t cv) {
  ...
  if(numtinit==0){interrupt_enable(-1)};
  lock_cv_t lock_cv = make_pair(lock, cv);
...
int thread_broadcast(lock_t lock, lock_t cv) {
  ...
  lock_cv_t lock_cv = make_pair(lock, cv);
  broadcasting_status[running] = true;
  // if lock doesn't even exist within our knowledge, initialize it to NULL
  if(lock_owner.find(lock) == lock_owner.end()) lock_owner[lock] = NULL;
  ...
  broadcasting_status[running] = false;
